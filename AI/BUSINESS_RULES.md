...
*   **Blast gauge (Fúria):** Cada Digimon possui sua **própria** barra de Blast (0–1000). Não existe barra global compartilhada. Com 3 Digimons na party, cada um mantém seu valor individualmente — inclusive fora de batalha, quando o jogo não exibe a barra. Endereço em RAM: `0x00042B74 + (2 × rookieId)` (Int32 LE), onde `rookieId` é a chave numérica (0–7) em `DigimonsAddresses.json`.
