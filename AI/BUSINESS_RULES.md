...
graph TD
    subgraph Backend [Backend - C# .NET]
        A[Emulador Duckstation] -->|Leitura de RAM 500ms| B(MemoryReader / MemoryBlockReader)
        B --> L[Loaders]
...
*   **Frequência:** A cada `500ms`, a classe `GameLoopService` executa um loop de varredura de memória (dentro de uma instrução `while`).
...
*   **Leitura de Memória:** O backend lê apenas as propriedades dinâmicas `Value` (byte) de cada `Step`/`Requisite` na RAM; no modelo de domínio e no contrato SignalR isso vira `IsDone` (bool).
*   **Cascade da Main Quest:** Em `MainQuestAssembler`, após o mapeamento Resource → Domain, o backend aplica cascade de progressão: se um step posterior está `IsDone` e um step anterior ainda não, esse anterior é forçado para `IsDone = true` (percorrendo os steps de trás para frente). Isso compensa um quirk da memória do jogo em que steps intermediários podem permanecer `0` mesmo com progresso posterior já marcado. **Escopo:** somente a Main Quest. Side quests, legendary weapons e DRI agents são pass-through puro via `QuestAssembler` — não aplicar o mesmo cascade sem decisão explícita de produto. O frontend pode, portanto, ver `IsDone` coerentes na main quest que não batem byte a byte com a RAM em steps antigos.
*   **Normalização Duel Island:** Em `DuelIslandAssembler`, cada quest da categoria: (1) se algum requisite não está `IsDone`, todos os steps são forçados para `IsDone = false`; (2) se o último step (troféu) está `IsDone`, os steps anteriores são forçados para `IsDone = true`. **Escopo:** somente Duel Island.
