...
*   **Cascade da Main Quest:** Após montar a missão principal a partir da RAM, o backend aplica uma normalização de progressão (`NormalizeMainQuestProgression`): se um step posterior está `IsDone` e um step anterior ainda não, esse anterior é forçado para `IsDone = true` (percorrendo os steps de trás para frente). Isso compensa um quirk da memória do jogo em que steps intermediários podem permanecer `0` mesmo com progresso posterior já marcado. **Escopo:** somente a Main Quest. Side quests, legendary weapons e DRI agents são pass-through puro dos valores lidos (após bitmask → `IsDone`) — não aplicar o mesmo cascade sem decisão explícita de produto. O frontend pode, portanto, ver `IsDone` coerentes na main quest que não batem byte a byte com a RAM em steps antigos.
