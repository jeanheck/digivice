        ...
        public void Render(Player? player, Party party)
        {
            ...
            else
            {
                for (int i = 0; i < party.Digimons.Count; i++)
                {
                    ...
                    Console.WriteLine($"Slot {d.SlotIndex}: {d.Name.PadRight(NamePadding)} [Lvl: {d.Level.ToString(LvlFormat)}] [EXP: {d.Experience.ToString(ExpFormat)}] HP: {d.CurrentHP.ToString(StatFormat)}/{d.MaxHP.ToString(StatFormat)} MP: {d.CurrentMP.ToString(StatFormat)}/{d.MaxMP.ToString(StatFormat)}");
