        ...
        public void Render(Player? player)
        {
            ...
            else
            {
                for (int i = 0; i < player.Party.Digimons.Count; i++)
                {
                    ...
                    var b = d.BasicInfo;
                    Console.WriteLine($"Slot {d.SlotIndex}: {d.Name.PadRight(NamePadding)} [Lvl: {b.Level.ToString(LvlFormat)}] [EXP: {b.Experience.ToString(ExpFormat)}] HP: {b.CurrentHP.ToString(StatFormat)}/{b.MaxHP.ToString(StatFormat)} MP: {b.CurrentMP.ToString(StatFormat)}/{b.MaxMP.ToString(StatFormat)}");
