...
| `Optional<T>` sem `IEquatable` | ~~implícito via struct~~ → `IEquatable<Optional<T>>` + `==`/`!=` (**feito**) |
...
| P3-2 | Helper anti-boilerplate nos Differs | Preferência de produto: `if` + `with` explícitos são mais legíveis; DRY rejeitado |
