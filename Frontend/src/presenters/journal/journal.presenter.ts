...
export class JournalPresenter {
    public static getJournalViewModel(journal: Journal, digimonSlots: DigimonSlot[]): JournalViewModel {
        ...
        const sideQuestsViewModels = QuestRepository.getSideQuestsRaw()
            .map((sideQuestRaw) => QuestModalPresenter.getQuestViewModel(journal, sideQuestRaw.id, partyLevel))
        ...
        const legendaryWeaponsViewModels = QuestRepository.getLegendaryWeaponsRaw()
            .map((legendaryWeaponRaw) => QuestModalPresenter.getQuestViewModel(journal, legendaryWeaponRaw.id, partyLevel))
            .filter((questViewModel): questViewModel is QuestViewModel => questViewModel !== null);

        const driAgentsViewModels = QuestRepository.getDriAgentsRaw()
            .map((driAgentRaw) => QuestModalPresenter.getQuestViewModel(journal, driAgentRaw.id, partyLevel))
            .filter((questViewModel): questViewModel is QuestViewModel => questViewModel !== null);
