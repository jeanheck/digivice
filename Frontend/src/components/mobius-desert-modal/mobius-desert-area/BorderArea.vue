...
const SOURCE_TYPE: DesertAreaTypeViewModel = "border";
...
<template>
  <div
    class="relative flex w-16 h-16 items-center justify-center bg-gray-700"
    :class="clickable ? 'cursor-pointer hover:brightness-110' : 'cursor-default'"
    @click="onClick"
  >
    <span
      v-if="label"
      class="relative z-10 overflow-hidden px-0.0 text-center font-['Exo_2'] font-bold leading-tight text-[20px] text-gray-500"
    >
      {{ label }}
    </span>
    <span
