...
public class DigievolutionDifferTests
{
    ...
    {
        var previous = new Digievolution { Level = 15, Dvexp = 100 };
        var newObj = new Digievolution { Level = 15, Dvexp = 100 };
    ...
    {
        var newObj = new Digievolution { Level = 25, Dvexp = 300 };
        ...
        Assert.True(result.Dvexp.HasValue);
        Assert.Equal(300, result.Dvexp.Value);
    }
    ...
    {
        var previous = new Digievolution { Level = 15, Dvexp = 100 };
        var newObj = new Digievolution { Level = 16, Dvexp = 100 };
        ...
        Assert.False(result.Dvexp.HasValue);
    }

    [Fact]
    public void Diff_ShouldReturnChangedDvexp_WhenOnlyDvexpChanged()
    {
        var previous = new Digievolution { Level = 15, Dvexp = 100 };
        var newObj = new Digievolution { Level = 15, Dvexp = 144 };
        ...
        Assert.True(result.Dvexp.HasValue);
        Assert.Equal(144, result.Dvexp.Value);
    }
}
